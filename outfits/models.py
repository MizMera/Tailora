...
class Outfit(models.Model):
    ...
    def mark_as_worn(self):
        """Mark this outfit as worn today and update statistics"""
    ...
    def get_item_categories(self):
        """Get all categories of items in this outfit"""
        return list(set(item.category.name for item in self.items.all() if item.category))
    
    def get_colors(self):
        """Get all colors present in this outfit"""
        return list(set(item.color for item in self.items.all() if item.color))
    
    def get_last_worn_status(self):
        """Get status of when this was last worn"""
        if not self.last_worn:
            return "never"
        ...
        days_ago = (today - self.last_worn).days
